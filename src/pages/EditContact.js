...
import { updateContact } from "../state/redux/actions/contacts/update";
import { fetchContacts } from "../state/redux/actions/contacts/fetch";


const EditContact = ({ openDialog, onClose, comName,comId,initialData }) => {
  console.log(comName);
  console.log(comId);
  ...
  useEffect(()=>{
    if(initialData){
      ...
      setContactRole(initialData.role||"")
  ...
  const handleClos = () => {
    ...
    dispatch(updateContact(newItem.id,newItem))
      .then(() => {
